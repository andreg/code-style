...
class BlankLineAroundEnumBodyFixer extends AbstractFixer {

public function getDefinition(): FixerDefinitionInterface {
		return new \PhpCsFixer\FixerDefinition\FixerDefinition(
			'Ensure there is one blank line before and after the enum body.',
			[]
		);
	}

	public function getName(): string {
		return 'Andreg/blank_line_around_enum_body';
	}
	...
	public function applyFix( \SplFileInfo $file, Tokens $tokens ): void {
		for ( $index = 0; $index < $tokens->count(); $index++ ) {
			if ( $tokens[ $index ]->isGivenKind( T_ENUM ) ) {
				// Find the opening and closing curly braces of the enum
